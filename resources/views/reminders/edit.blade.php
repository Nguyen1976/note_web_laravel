    ...
    <div class="flex justify-center mt-10">
        <div class="w-full max-w-md">
            <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="post"
                ...
                <div class="mb-4">
                    <label for="sent" class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="sent" name="sent" value="1"
                            class="form-checkbox text-blue-600"
                            {{ old('sent', $reminder->sent ?? false) ? 'checked' : '' }}>
                        <span class="ml-2 text-gray-700 text-sm font-medium">Sent</span>
                    </label>
                    @error('sent')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
